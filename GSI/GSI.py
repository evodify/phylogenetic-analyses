#!/usr/bin/env python2
"""
This script calculates genealogical sorting index (GSI) for a given list of taxa.
It iterates through all the trees in a file and outputs GSIs for each tree.

For more detail on GSI see:
Cummings et al. 2008. A Genealogical Approach to Quantifying Lineage Divergence.
Evolution, Vol. 62, No 9. pp.2411-2422
...
python2 GSI.py \
    -t tree.nwk \
    -o tree.out \
    -g "gr1[a1,a2,a3,a4];gr2[b1,b2,b3,b4]" \
    -p 1000 \
    -c tree.names
"""
...
parser.add_argument(
    '-t',
    '--tree',
    help='file containing trees in newick format',
    type=str,
    required=True)
parser.add_argument(
    '-c',
    '--coordinates',
    help='file indicating trees\' position on the genome',
    type=str,
    required=True)
parser.add_argument(
    '-o', '--output', help='output name', type=str, required=True)
parser.add_argument(
    '-g',
    '--groups',
    help='groups in the format "group1[taxon1,taxon2];group2[taxon4,taxo5]"',
    type=str,
    required=True)
parser.add_argument(
    '-p',
...
def count_subtree_internal_nodes(tree, group):
  ...
  '''
    ...
    for group_name in group:
        leaf = tree.search_nodes(
            name=group_name)[0]  # extract a leaf from a tree
...
def gsi(tree, group):
    '''
  obsGS = n/sum(du-2)
  where d is the degree of node u of U total nodes uniting a group (estimated
  coalescent events) through the MRCA.
  n is the minimum number of nodes (coalescent events) required to unite
  a group of size n + 1 through the MRCA.
  maxGS is the maximum possible GS. The maxGS is reached when a group is
  monophyletic.
  minGS = n/sum(di-2)
  where i is one of I total nodes on the tree. Thus minGS would result if all
  nodes on a tree were required to unite a group.
  ...
  '''
    ...
    minGS = float(n) / float(
        count_all_internal_nodes(tree)
    )  # modify for trees that include more leaves than specified in groups
...
if file_len(args.tree) != file_len(args.coordinates):
    raise IOError("%s and %s are of different length" % (args.tree,
                                                         args.coordinates))
...
for i in range(len(groups)):
    ...
    greter.append(
        0
    )  # make lists to store GSI values for the gsiT permutation list of lists
...
for tree, treeName in zip(treeFile, coordinatesFile):
    ...
    for indName in indNamesflat:
        if indName not in tree:
            raise ValueError("%s is not present in the tree %s" % (indName,
                                                                   tree))
...
if args.permutation:
    ...
    for k in range(pNperm):
        ...
        for gr in groupNames:
            gsiTp.append(
                0.0)  # to store GSI values for the gsiT permutation
